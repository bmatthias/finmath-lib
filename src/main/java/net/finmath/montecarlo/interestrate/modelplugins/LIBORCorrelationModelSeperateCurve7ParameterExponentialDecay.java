 ...
 */
public class LIBORCorrelationModelSeperateCurve7ParameterExponentialDecay extends AbstractLIBORCorrelationModelSeperateCurves {

    public LIBORCorrelationModelSeperateCurve7ParameterExponentialDecay(TimeDiscretizationInterface timeDiscretization, TimeDiscretizationInterface liborPeriodDiscretization, int numberOfFactors, double[] fixedParameter, boolean isCalibrateable) {
        this(timeDiscretization, liborPeriodDiscretization, numberOfFactors, fixedParameter, new double[] {
                0.1, 0.1, 0.1, 0.1
        }, isCalibrateable);
    }

    public LIBORCorrelationModelSeperateCurve7ParameterExponentialDecay(TimeDiscretizationInterface timeDiscretization, TimeDiscretizationInterface liborPeriodDiscretization, int numberOfFactors, double[] fixedParameter, double[] calibrationParameter, boolean isCalibrateable) {
    ...
    protected void generateCorrelationMatrices(int timeSteps, double[][] firstCurveCorrelationMatrix, double[][] secondCurveCorrelationMatrix, double[][] curvesCorrelationMatrix) {
        for (int row = 0; row < timeSteps; row++) {
            for (int col = row; col < timeSteps; col++) {
                ...
                double correlation3 = calibrationParameter[3];
                ...
                curvesCorrelationMatrix[col][row] = correlation3;
            }
